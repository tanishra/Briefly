...
import requests
from datetime import datetime
from AI_Agent_System.config import RESEND_API_KEY, RECIPIENT_EMAIL
...
# Main Resend Email Function
def send_html_email_with_charts(report_files, chart_files):
    if not RESEND_API_KEY:
        print("❌ Missing RESEND_API_KEY in config.")
        return False
    ...
    settings = get_user_settings()
    if settings and not settings.get('notifications_enabled', True):
        print("⚠️  Email notifications disabled in settings.")
        return False

    subject = f"📈 Daily Business Report - {datetime.now().strftime('%B %d, %Y')}"
    html_body = create_html_email_with_user_name(user_name)

    try:
        # Combine attachments
        files = []
        for path in (report_files + chart_files):
            if os.path.exists(path):
                files.append(('attachments', (os.path.basename(path), open(path, 'rb'))))
        ...
        response = requests.post(
            ...
            data={
                "from": f"Briefly AI <tanishrajput9@gmail.com>",  # Must be verified domain
                "to": recipient_email,
                "subject": subject,
                "html": html_body
            },
